...
export class AccessibilitySettingsComponent implements OnInit, OnDestroy {
  ...
  /**
   * Simple on/off options (used for other toggles)
   */
  binarySwitchOptions: SwitchOption[] = [
    { value: true, labelColor: SwitchColor.Success, backgroundColor: SwitchColor.Success, label: 'Enabled' },
  ...
  updateFormValues() {
    this.settingsService.getAll().pipe(take(1)).subscribe(storedSettings => {
      ...
      const settingsRequiringDefaultValue: Array<Extract<AccessibilitySetting, 'notificationTimeOut' | 'liveRegionTimeOut'>> = [
        'notificationTimeOut',
        'liveRegionTimeOut',
      ];
