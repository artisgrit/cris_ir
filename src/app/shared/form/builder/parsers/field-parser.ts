...
export abstract class FieldParser {
  ...
  /**
   * Some repository metadata (like access status) is meant to support multiple values
   * even when the submission configuration marks it as non-repeatable.
   */
  protected getEffectiveRepeatable(): boolean {
    return this.configData.repeatable || this.isAccessStatusMetadata();
  }

  protected isAccessStatusMetadata(): boolean {
    if (!Array.isArray(this.configData.selectableMetadata)) {
      return false;
    }
    return this.configData.selectableMetadata.some((entry) => entry?.metadata === 'dc.rights.access');
  }

  public parse() {
    const effectiveRepeatable = this.getEffectiveRepeatable();

    if (((this.getInitValueCount() > 1 && !effectiveRepeatable) || (effectiveRepeatable))
      ...
      const config = {
        ...
        notRepeatable: !effectiveRepeatable,
  ...
  protected initModel(id?: string, label = true, labelEmpty = false, setErrors = true, hint = true) {
    ...
    controlModel.repeatable = this.getEffectiveRepeatable();
