...
export class GeospatialMapComponent implements AfterViewInit, OnInit, OnDestroy {
  ...
  private initMap(): void {
    ...
    this.addLatLngReadoutControl(L);
  ...
  private addLatLngReadoutControl(L): void {
    const latLngControl = L.control({ position: 'bottomleft' });
    let controlEl;
    ...
    latLngControl.onAdd = () => {
      controlEl = L.DomUtil.create('div', 'latlng-control leaflet-bar');
      controlEl.innerHTML = formatLatLng(this.DEFAULT_CENTRE_POINT[0], this.DEFAULT_CENTRE_POINT[1]);
      return controlEl;
    };
    ...
    this.map.on('mousemove', (e) => {
      if (controlEl && e?.latlng) {
        controlEl.innerHTML = formatLatLng(e.latlng.lat, e.latlng.lng);
      }
    });
    ...
    this.map.on('moveend', () => {
      if (controlEl) {
        ...
        controlEl.innerHTML = formatLatLng(center.lat, center.lng);
      }
    });
  }
  ...
  private renderBoundingBox(L, bbox: string): number[][] {
    if (hasValue(bbox)) {
      ...
      const defaultStrokeColor = '#0b3d91';
      const hoverStrokeColor = '#d7014d';
      const boundingBox = L.rectangle(bounds, { color: defaultStrokeColor, weight: 2, fill: false });
      boundingBox.on('mouseover', () => {
        boundingBox.setStyle({ color: hoverStrokeColor });
      });
      boundingBox.on('mouseout', () => {
        boundingBox.setStyle({ color: defaultStrokeColor });
      });
